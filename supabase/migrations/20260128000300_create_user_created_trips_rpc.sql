  ...
  RETURN QUERY EXECUTE format('
    ...
    WHERE t.created_by = $1
      ...
      AND (%s = ''전체'' OR EXISTS (
        ...
        JOIN regions reg ON reg.id = tr.region_id
        WHERE tr.trip_id = t.id AND reg.name = %s
      ))
      AND (%s = ''전체'' OR EXISTS (
        SELECT 1 FROM trip_themes tt
        JOIN themes thm ON thm.id = tt.theme_id
        WHERE tt.trip_id = t.id AND thm.name = %s
      ))
      AND (%s IS NULL OR t.created_at >= NOW() - make_interval(days => %s))
    ORDER BY %s
    LIMIT $2
  ', cursor_condition, p_region, p_region, p_theme, p_theme, p_days, p_days, sort_clause)
