...
const MOCK_TRIPS = [
  ...
  { id: 1, title: "제주도 식도락 여행 🍊", description: "제주 맛집 완전 정복 코스", start_date: "2024-02-10", end_date: "2024-02-13", cover_image_url: "https://images.unsplash.com/photo-1542662565-7e4b66bae529?w=500&q=60", region: "제주", theme: "미식", author: { name: "여행자1" }, like_count: 12, bookmark_count: 5, member_count: 4, created_at: "2024-01-20T10:00:00", isLiked: false, isBookmarked: false },
  { id: 2, title: "부산 2박 3일 힐링 🌊", description: "바다 보며 물멍 때리기", start_date: "2024-03-01", end_date: "2024-03-03", cover_image_url: "https://images.unsplash.com/photo-1621845199676-787140c94609?w=500&q=60", region: "부산", theme: "힐링", author: { name: "BusanLover" }, like_count: 24, bookmark_count: 8, member_count: 2, created_at: "2024-01-25T14:30:00", isLiked: false, isBookmarked: false },
  { id: 3, title: "도쿄 벚꽃 여행 🌸", description: "봄바람 휘날리며", start_date: "2024-04-05", end_date: "2024-04-09", cover_image_url: "https://images.unsplash.com/photo-1540959733332-eab4deabeeaf?w=500&q=60", region: "도쿄", theme: "관광", author: { name: "J-Pop" }, like_count: 45, bookmark_count: 20, member_count: 3, created_at: "2024-02-01T09:00:00", isLiked: false, isBookmarked: false },
  { id: 4, title: "강릉 커피 투어 ☕", description: "커피 향 가득한 여행", start_date: "2024-01-25", end_date: "2024-01-26", cover_image_url: "https://images.unsplash.com/photo-1627447186259-fc53907c6f09?w=500&q=60", region: "강릉", theme: "미식", author: { name: "Coffee" }, like_count: 8, bookmark_count: 2, member_count: 2, created_at: "2024-02-10T11:20:00", isLiked: false, isBookmarked: false },
  { id: 5, title: "뉴욕 도심 탐방 🗽", description: "잠들지 않는 도시", start_date: "2024-05-10", end_date: "2024-05-17", region: "뉴욕", theme: "관광", cover_image_url: "https://images.unsplash.com/photo-1496442226666-8d4d0e62e6e9?w=500&q=60", author: { name: "NY_Lover" }, like_count: 30, bookmark_count: 12, member_count: 1, created_at: "2024-02-15T16:45:00", isLiked: false, isBookmarked: false },
  { id: 6, title: "파리 낭만 여행 🥖", description: "에펠탑 아래 피크닉", start_date: "2024-06-01", end_date: "2024-06-07", region: "파리", theme: "힐링", cover_image_url: "https://images.unsplash.com/photo-1502602898657-3e91760cbb34?w=500&q=60", author: { name: "Bonjour" }, like_count: 55, bookmark_count: 22, member_count: 2, created_at: "2024-02-20T10:00:00", isLiked: false, isBookmarked: false },
  { id: 7, title: "방콕 먹방 투어 🍜", description: "팟타이와 똠양꿍", start_date: "2024-07-15", end_date: "2024-07-20", region: "방콕", theme: "미식", cover_image_url: "https://images.unsplash.com/photo-1552465011-b4e21bf6e79a?w=500&q=60", author: { name: "ThaiFood" }, like_count: 18, bookmark_count: 4, member_count: 4, created_at: "2024-02-25T12:00:00", isLiked: false, isBookmarked: false },
  { id: 8, title: "스위스 알프스 🏔️", description: "대자연 속으로", start_date: "2024-08-10", end_date: "2024-08-18", region: "스위스", theme: "힐링", cover_image_url: "https://images.unsplash.com/photo-1530122037265-a5f1f91d3b99?w=500&q=60", author: { name: "Alps" }, like_count: 40, bookmark_count: 15, member_count: 2, created_at: "2024-03-01T09:00:00", isLiked: false, isBookmarked: false },
];
      ...
      <div className="position-relative group">
        ...
        >
          {trips.length > 0 ? (
            ...
            <div className="w-100 text-center py-5 text-muted bg-light rounded-4">조건에 맞는 여행지가 없습니다. 🐢</div>
          )}
        </div>
...
export default function HomePage() {
  ...
  const filteredTrips = useMemo(() => {
    return trips.filter(trip => {
      ...
      // 2. 지역 필터
      const matchRegion = selectedRegion === "전체" || trip.region === selectedRegion;
      // 3. 테마 필터
      const matchTheme = selectedTheme === "전체" || trip.theme === selectedTheme;
  ...
  const handleLike = (id) => {
    setTrips(prev => prev.map(t => t.id === id ? { ...t, isLiked: !t.isLiked, like_count: !t.isLiked ? t.like_count + 1 : t.like_count - 1 } : t));
  };

  const handleBookmark = (id) => {
    setTrips(prev => prev.map(t => t.id === id ? { ...t, isBookmarked: !t.isBookmarked, bookmark_count: !t.isBookmarked ? t.bookmark_count + 1 : t.bookmark_count - 1 } : t));
  };

  // ✅ [수정] 필터링된 결과(filteredTrips)를 기준으로 최신순 정렬
  const recentTrips = [...filteredTrips].sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
    ...
    <div className="pb-5 bg-white">
      ...
      <div className="hero-section mb-5">
        <Container className="text-center text-white">
          ...
          <h1 className="display-4 fw-bolder mb-3">당신의 다음 여행은 어디인가요?</h1>
          
          <Row className="justify-content-center">
            <Col md={8} lg={6}>
              <InputGroup size="lg" className="mb-3">
                <Form.Control 
                  placeholder="여행지, 태그, 키워드로 검색" className="search-input py-3"
      ...
      <Container>
        ...
        <TripSection title="내 관심사에 맞는 여행지를 찾아보세요" subtitle="무엇을 좋아하시나요?" trips={filteredTrips} onLike={handleLike} onBookmark={handleBookmark} />
        <TripSection title="인기 테마" subtitle="트렌디한 테마 여행을 만나보세요" trips={[...filteredTrips].reverse()} onLike={handleLike} onBookmark={handleBookmark} />
        <TripSection title="인기 여행지" subtitle="가장 많은 사랑을 받은 여행지입니다" trips={filteredTrips} onLike={handleLike} onBookmark={handleBookmark} />
        <TripSection title="최근 등록된 여행지" subtitle="따끈따끈한 여행 계획들입니다." trips={recentTrips} onLike={handleLike} onBookmark={handleBookmark} />

        <div className="rounded-4 p-5 mt-5 text-center text-white position-relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', boxShadow: '0 10px 20px rgba(118, 75, 162, 0.3)' }}>
           <div className="position-relative z-1">
             <h2 className="fw-bold mb-3">나만의 여행을 계획할 준비가 되셨나요?</h2>
             <Button variant="light" size="lg" className="rounded-pill px-5 fw-bold text-primary shadow-sm" as={Link} to="/trips/new">여행 일정 만들기 ✨</Button>
           </div>
