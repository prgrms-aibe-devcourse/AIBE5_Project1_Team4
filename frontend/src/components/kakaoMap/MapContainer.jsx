...
const MapContainer = ({ mapCurrentDayPos = [], mapSearchPlacePos = [] }) => {
  ...
  // 2. 경로 계산 훅
  const { routeData, calculateRoute, loading: isRouteLoading } = useKakaoRoute();
  ...
  const center = useMemo(() => {
    ...
    if (firstItinerary) return { lat: firstItinerary.lat, lng: firstItinerary.lng };
  ...
  // --- [로딩 및 에러 상태 처리] ---
  if (loading) return (
    <div className="map-loading-state" style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f8f9fa' }}>
      <p>지도를 불러오는 중입니다...</p>
    </div>
  );
  
  if (error) return (
    <div className="map-error-state" style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fff0f0' }}>
  ...
  return (
    ...
    >
      ...
      {/* --- [4] 구간 정보 --- */}
      {!isRouteLoading && routeData.sections.map((section, index) => (
        <CustomOverlayMap 
